...
class FileDownloader:
    ...
    def __init__(self, url: str, outbound_directory: str) -> None:
        ...
        self.binary_bytes: str = ONE_MB_IN_BINARY_BYTES

    def download(self) -> None:
        headers = {}

        # append modified header only if we already have file locally stored
        if os.path.exists(self.dataset_file_dir):
            modified_time = os.path.getmtime(self.dataset_file_dir)
        ...
        if response.status_code == requests.codes.ok:
            ...
            with open(self.dataset_file_dir, "wb") as file:
                for chunk in response.iter_content(chunk_size=self.binary_bytes):
                    file.write(chunk)

            # fetch and store the new modified header for the next download
